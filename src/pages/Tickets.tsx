...
export default function Tickets() {
  ...
  useEffect(() => {
    updateTotal(filteredTickets.length);
    if (currentPage > Math.ceil(filteredTickets.length / itemsPerPage)) {
      goToPage(1);
    }
  }, [filteredTickets.length, currentPage, updateTotal, goToPage]);

  const getStatusBadgeVariant = (status: Ticket['status']) => {
    switch (status) {
      case 'submitted':
      case 'in_review':
        return 'default'; // Blue for open
      case 'in_progress':
        return 'secondary'; // Yellow for in progress
      case 'resolved':
      case 'closed':
        return 'default'; // Green for resolved - we'll add custom styling
      case 'pending_client':
        return 'outline';
      default:
        return 'outline';
