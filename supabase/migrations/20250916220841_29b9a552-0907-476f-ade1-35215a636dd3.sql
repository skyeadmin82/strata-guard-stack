-- Create missing sales tables and populate with sample data
...
-- Add missing columns to sales_agents if they don't exist
DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN quota_monthly NUMERIC DEFAULT 0;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN quota_quarterly NUMERIC DEFAULT 0;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN quota_annual NUMERIC DEFAULT 0;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN max_leads INTEGER DEFAULT 50;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN current_lead_count INTEGER DEFAULT 0;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN performance_score NUMERIC DEFAULT 0;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN territory JSONB DEFAULT '{}';
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE sales_agents ADD COLUMN specializations TEXT[] DEFAULT '{}';
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;
...
-- Create sales_deals table if it doesn't exist
...
-- Create lead distribution rules table if it doesn't exist
...
-- Insert sample sales agents (update existing or insert new)
INSERT INTO sales_agents (tenant_id, first_name, last_name, email, phone, agent_code, tier, quota_monthly, quota_quarterly, quota_annual, commission_rate, max_leads, performance_score, territory, specializations) VALUES
((SELECT get_current_user_tenant_id()), 'John', 'Smith', 'john.smith@company.com', '555-0101', 'JS001', 'platinum', 50000, 150000, 600000, 15.0, 75, 95.2, '{"region": "North", "states": ["NY", "NJ", "CT"]}', ARRAY['Enterprise', 'Cybersecurity']),
((SELECT get_current_user_tenant_id()), 'Sarah', 'Johnson', 'sarah.johnson@company.com', '555-0102', 'SJ002', 'gold', 35000, 105000, 420000, 12.0, 60, 87.8, '{"region": "South", "states": ["TX", "FL", "GA"]}', ARRAY['SMB', 'Cloud Services']),
((SELECT get_current_user_tenant_id()), 'Mike', 'Davis', 'mike.davis@company.com', '555-0103', 'MD003', 'silver', 30000, 90000, 360000, 10.0, 50, 76.5, '{"region": "West", "states": ["CA", "WA", "OR"]}', ARRAY['Healthcare', 'Compliance']),
((SELECT get_current_user_tenant_id()), 'Lisa', 'Wilson', 'lisa.wilson@company.com', '555-0104', 'LW004', 'bronze', 25000, 75000, 300000, 8.0, 40, 68.3, '{"region": "Midwest", "states": ["IL", "OH", "MI"]}', ARRAY['Manufacturing', 'Retail'])
ON CONFLICT (tenant_id, agent_code) DO UPDATE SET
    quota_monthly = EXCLUDED.quota_monthly,
    quota_quarterly = EXCLUDED.quota_quarterly,
...
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE agent_code = 'JS001' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Enterprise Security Package', 50000, 80, 'negotiation', CURRENT_DATE + INTERVAL '15 days', 15.0, 'Large enterprise deal, high priority'),
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE agent_code = 'SJ002' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Cloud Migration Project', 30000, 65, 'proposal', CURRENT_DATE + INTERVAL '30 days', 12.0, 'SMB cloud transformation'),
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE agent_code = 'MD003' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Compliance Audit Services', 20000, 90, 'closed_won', CURRENT_DATE - INTERVAL '5 days', 10.0, 'Successfully closed healthcare compliance deal'),
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE agent_code = 'LW004' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Retail Security Assessment', 15000, 45, 'needs_analysis', CURRENT_DATE + INTERVAL '45 days', 8.0, 'Retail chain security evaluation');
