...
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE email = 'john.smith@company.com' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Enterprise Security Package', 50000, 80, 'negotiation', CURRENT_DATE + INTERVAL '15 days', 15.0, 'Large enterprise deal, high priority'),
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE email = 'sarah.johnson@company.com' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Cloud Migration Project', 30000, 65, 'proposal', CURRENT_DATE + INTERVAL '30 days', 12.0, 'SMB cloud transformation'),
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE email = 'mike.davis@company.com' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Compliance Audit Services', 20000, 90, 'closed_won', CURRENT_DATE - INTERVAL '5 days', 10.0, 'Successfully closed healthcare compliance deal'),
((SELECT get_current_user_tenant_id()), (SELECT id FROM sales_agents WHERE email = 'lisa.wilson@company.com' AND tenant_id = get_current_user_tenant_id() LIMIT 1), 'Retail Security Assessment', 15000, 45, 'needs_analysis', CURRENT_DATE + INTERVAL '45 days', 8.0, 'Retail chain security evaluation');
