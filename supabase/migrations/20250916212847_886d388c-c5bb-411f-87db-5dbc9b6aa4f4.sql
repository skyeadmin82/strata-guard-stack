...
FOR UPDATE USING (tenant_id = get_current_user_tenant_id() AND user_has_role('admin'::user_role));
...
CREATE POLICY "Tenant can view system metrics" ON public.system_metrics
FOR SELECT USING (tenant_id = get_current_user_tenant_id());

CREATE POLICY "Admins can insert system metrics" ON public.system_metrics
FOR INSERT WITH CHECK (tenant_id = get_current_user_tenant_id() AND user_has_role('admin'::user_role));

CREATE POLICY "Admins can update system metrics" ON public.system_metrics
FOR UPDATE USING (tenant_id = get_current_user_tenant_id() AND user_has_role('admin'::user_role));
